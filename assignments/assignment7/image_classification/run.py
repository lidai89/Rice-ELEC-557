...
def run(grad,view,pixels,maxIter,numTrain):
    ...
    if pixels is False:
        ...
        centroids = submission3.runKMeans(k,patches,maxIter_kMeans)
        print "Finished running K-means"
        if view:
            util.viewPatches(centroids)
        trainX,trainY = util.kMeansFeatures(trainSImages,centroids,submission3.extractFeatures)
        testX, testY = util.kMeansFeatures(testImages,centroids,submission3.extractFeatures)
    else:
        maxIter = 100
...
def main(args=None):
    ...
    if opts.grad=="perceptron":
        ...
        grad = submission3.logisticGradient
        maxIter = 300
    elif opts.grad=="hinge":
        grad = submission3.hingeLossGradient
        maxIter = 300
    else:
